...
const tips: { title: string; text: string; icon: LucideIcon }[] = [
  {
    ...
    text: "Vpíchněte teploměr do středu nejtlustší části masa, mimo kost a tuk.",
    icon: Crosshair,
  },
  {
    title: "Carry-over efekt",
    text: "Sundejte maso 2–3 °C pod cílovou teplotu. Při odpočinku teplota stoupne.",
    icon: TrendingUp,
  },
  {
    title: "Nechte maso odpočinout",
    text: "Steak nechte 5–10 minut pod alobalem. Šťávy se lépe rozloží v celém kusu.",
    icon: Timer,
  },
  {
    title: "Kvalitní teploměr",
    text: "Instantní digitální teploměr je nejlepší investice grillmastera.",
    icon: Thermometer,
  },
];
...
const formatTemp = (temps: number[], unit: "C" | "F") => {
  if (unit === "F") {
    return temps.length === 2 ? `${toF(temps[0])}–${toF(temps[1])} °F` : `${toF(temps[0])} °F`;
  }
  return temps.length === 2 ? `${temps[0]}–${temps[1]} °C` : `${temps[0]} °C`;
};

const getBarWidth = (temps: number[]) => {
  const avg = (temps[0] + (temps[1] ?? temps[0])) / 2;
            ...
            <div className="self-start md:self-auto inline-flex items-center rounded-full p-1"
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-sm font-semibold transition-all ${
                  unit === "C" ? "text-white shadow-sm" : "text-stone hover:text-coal"
                }`}
                style={unit === "C" ? { backgroundColor: "var(--heat)" } : undefined}
              >
              ...
              <button
                onClick={() => setUnit("F")}
                className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-sm font-semibold transition-all ${
                  unit === "F" ? "text-white shadow-sm" : "text-stone hover:text-coal"
                }`}
                style={unit === "F" ? { backgroundColor: "var(--heat)" } : undefined}
              >
                °F
              </button>
            </div>
